...
public class CustomDrawableView extends LinearLayout {
    ...
    protected static final float DEFAULT_POINTER_HALO_BORDER_WIDTH = 15f;
    ...
    protected static final int DEFAULT_CIRCLE_COLOR = Color.parseColor("#f38120");
...
//        mCirclePaint.setShader(new LinearGradient(0, 0, 100 , 100, new int[] {
//                Color.parseColor("#3b9ec9"),
//                Color.parseColor("#9ab477"),
//                Color.parseColor("#f38120"),
//                Color.parseColor("#3b9ec9"),
//                Color.parseColor("#9ab477"),
//                Color.parseColor("#f38120")},
//                new float[] {
//                        0,0.2f,0.4f,0.6f,0.8f,1 }, Shader.TileMode.CLAMP));
    ...
    protected void calculatePointerAngle() {
        ...
        calculateProgress(mPointerPosition);
    }

    protected void calculateProgress(float currentAngle){
        float progress = 0;
        if(currentAngle>=135 && currentAngle<=360){
            progress = (currentAngle - 135)/(mTotalCircleDegrees/100);
        } else if(currentAngle>=0 && currentAngle<=45){
            progress = (currentAngle + 225)/(mTotalCircleDegrees/100);
        }
        calculateAngle(Math.round(progress));
        Log.d("Abinay mPointerPosition", String.valueOf(Math.round(progress)));
    }

    protected void calculateAngle(int progressValue){
        float currentAngle = 135;
        currentAngle = currentAngle + progressValue*(mTotalCircleDegrees/100);
        if(currentAngle > 360){
            currentAngle = currentAngle - 360;
        }
        Log.d("Abinay calculateAngle", String.valueOf(currentAngle));
    }
    ...
    protected void init(AttributeSet attrs, int defStyle) {
        ...
        initPaints();
        showTextViewProgress("abinay",attrArray);
    }

    protected void showTextViewProgress(String progressValue, TypedArray attrArray){
        LayoutInflater inflater = (LayoutInflater) mContext
                .getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        LinearLayout linear = (LinearLayout) inflater.inflate(R.layout.progress_text, this, true);
        mProgressTextView = linear.findViewById(R.id.progress_text);
        linear.setGravity(Gravity.CENTER);
        mProgressTextView.setText(progressValue);
        Log.d("Abinay", "showTextViewProgress");
        attrArray.recycle();
    }
    ...
    public interface OnCircularSeekBarChangeListener {
        ...
        public abstract void onProgressChanged(CustomDrawableView circularSeekBar, int progress, boolean fromUser);

        public abstract void onStopTrackingTouch(CustomDrawableView seekBar);

        public abstract void onStartTrackingTouch(CustomDrawableView seekBar);
    }
